...
//PURPOSE: pops all elements from the stack to make it empty if it is not empty yet.
 void stack::clearIt()
 {
   if(!isEmpty()) //checks to see if it is not empty so it has to be false
     {
       el_t clear;//creates an object of el_t to use as local variable

       while(top!=-1)// pops until it gets to -1
	 {
	   pop(clear); //pops the local variable
	 }
